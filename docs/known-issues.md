...
| Regular image test is fixture-only | MBR/FAT/ext4 construction and read-only inspection pass, but the configured full root has not been imaged or booted | Apply operator credentials to a clone/prepared root, build the full image, then use read-only media preflight and fresh SD only |
