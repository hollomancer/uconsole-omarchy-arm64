...
| Prepared desktop image is plan-only | The 8 GiB geometry and dedicated build/inspection runner pass exact input checks, but Docker has only 2.0 GiB free and the host 2.7 GiB; the project output volume is empty and no image was allocated | Free enough disposable storage to leave at least 6 GiB in Docker (10 GiB recommended), run the explicit synthetic-image build, inspect it read-only, then delete the synthetic artifact |
