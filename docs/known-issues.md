...
| DKMS candidate is build-proven but not hardware-proven on ALARM | All modules/overlays compile for current `linux-rpi` and `linux-rpi-16k`, but load/probe/display/audio/battery and upgrade rollback remain untested on the uConsole | Keep runtime UNKNOWN and compare with the custom kernel on a recoverable development card |
