...
| Selected DKMS layer is build-proven but not hardware-proven on ALARM | All modules/overlays compile for current `linux-rpi-16k`, but load/probe/display/audio/battery and upgrade rollback remain untested on the uConsole | Keep runtime UNKNOWN and retain the custom kernel only as a differential oracle/recovery fallback |
