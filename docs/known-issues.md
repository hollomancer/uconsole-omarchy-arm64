...
| Minimal access/network policy awaits user input | The source `root` and `alarm` passwords remain unlocked; no project-owned SSH key, Wi-Fi credential or admin account name can be inferred safely | Require an explicit SSH public key and bootstrap-network choice, then lock both defaults before image construction |
...
| Regular image test is fixture-only | MBR/FAT/ext4 construction and read-only inspection pass, but the full prepared root has not been imaged or booted | Close the credential/network gate, build the full image, then use read-only media preflight and fresh SD only |
