...
| Prepared desktop image is plan-only | The 8 GiB geometry passes exact hardware/base/Hyprland/Omarchy state and headroom checks, but Docker has only 2.0 GiB free and the host 2.7 GiB; no new image was allocated | Free enough disposable storage to leave at least 6 GiB in Docker, build into a project-only volume, inspect it read-only, then delete the synthetic artifact |
