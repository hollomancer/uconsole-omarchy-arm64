...
| DKMS source emits compiler/DT warnings | Unused locals, two integer format mismatches and four unit-address root causes reduce confidence even though both builds and exact overlay application pass; the four DT nodes produce 14 differential warnings in the merged tree | Record exact warnings, fix/reconcile upstream, and do not suppress them in packaging |
...
| Custom uConsole modules are not in the first-boot initramfs | Panel, backlight, power and audio modules are available after the root filesystem mounts, but early panel output cannot be inferred | Root-storage/VC4/V3D initramfs closure passes; keep early display as WARN and rely on serial/SSH recovery for first boot |
...
| QEMU cannot model this target | Current QEMU documents Raspberry Pi machines only through Pi 4, not BCM2712/Pi 5/CM5; generic `virt` uses different firmware, kernel and devices | Use generic virtualization only for narrowly labeled userspace regressions; never accept it for the Phase 1 boot, display, GPU or peripheral gates |
