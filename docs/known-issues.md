...
| Real access/network inputs await user selection | The retained source is intentionally unchanged; admin identity, SSH key, regulatory domain, console recovery hash and optional Wi-Fi keyfile cannot be inferred safely | The file-only configuration and synthetic-image transactions pass; collect the operator's private inputs, plan, apply, and inspect on a fresh clone before building a development image |
...
| Whole image is not byte-reproducible | Two independent builds have different FAT, ext4 and complete-image digests even though every compared path, type, mode, owner, size, file digest, symlink target and normalized manifest field matches | Decide whether semantic repeatability is sufficient; if exact bytes are required, localize and normalize filesystem timestamps, seeds and allocation metadata before development-image promotion |
