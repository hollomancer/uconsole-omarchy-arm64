...
| Five core source packages remain unbuilt | `omacalc`, `omacut`, `ttf-ia-writer`, `ttfx` and `yaru-icon-theme` are explicitly classified but still lack promoted ARM artifacts | Build reproducibly off target, inspect payload architecture/license, then add to the future signed repository |
