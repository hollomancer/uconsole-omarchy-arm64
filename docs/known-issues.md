...
| Prepared desktop image is plan-only | The 8 GiB geometry and dedicated build/inspection runner pass immutable input checks, but the latest output-volume preflight found only 483,184 KiB free and the host had 1,139,840 KiB; it exited 3 before allocation | Free enough disposable storage to leave at least 6 GiB in Docker (10 GiB recommended), rerun `--check`, then run the explicit synthetic-image build and read-only inspection |
| External image output is not yet exercised | `SSDmini` has 897 GiB free and the runner now accepts a pre-created namespaced directory there, but Docker Desktop bind access and loop-device support for a file on that mount are not yet proven | With operator approval, create only the named project directory, run `--check`, then perform a small disposable loop-file probe before the 8 GiB build |
